...
- **Trending local talk / buzz** — what Singaporeans are actually talking about right now: viral incidents, human-interest and quirky local stories, celebrity/entertainment chatter, neighbourhood happenings, social-media flashpoints. This is an **expected, recurring part of the section, not an afterthought** — there is **no cap**: card **every** qualifying trending item in the window (see "Trending & human-interest items" guidance below).
...
The Singapore section should not be all hard policy and crime — readers also want the **buzz**: the viral, quirky, human-interest, and entertainment stories Singaporeans are actually discussing. **There is no cap on these items — include ALL of them that qualify.** Walk the whole Mothership feed every run and card every in-window item that passes the gates below; zero only if genuinely nothing is circulating. These sit alongside the hard-news cards, not instead of them.

- **Mothership is the lead source here, and it is collated in full.** Mothership (`mothership.sg/feed/`, via the workaround in 2b) is exactly where this buzz surfaces first and often *only* — its unique items (a viral spat, a celebrity death, a neighbourhood happening, a fintech feature) are the ones that previously got dropped. **Read the entire feed, not the top few entries**, and card every item inside the 24h window that clears the recency/reality/taste gates — do not sample, do not pick "the best 3", do not truncate the list because the section is already long. For a trending/human-interest card, **Mothership may be the primary/sole outlet** — do NOT discard it just because CNA/ST didn't also run it. Other good sources: CNA Lifestyle/TODAY, The Straits Times Life, and reputable local outlets.
...
- **Balance by ordering, not by dropping.** Hard news (policy, advisories, crime, foreign relations) still **leads** the section — put those cards first, then the trending/buzz cards below them. With the cap removed, balance is achieved by card order, NOT by deleting trending items to keep the section short. A long Singapore section on a busy Mothership day is the intended outcome.
- **Still dedupe.** If several feed entries (Mothership + CNA/ST, or two Mothership follow-ups) cover the **same** event, that's ONE card with the best source — "include all trending items" means all distinct stories, not duplicate cards for one story.
- **What still gets dropped:** anything outside the 24h window, unverifiable rumour/"netizens speculate" content, pure gossip with no public interest, pile-ons or harassment of a named private individual, and pure advertorial/sponsored posts. Everything else in the feed gets a card.
...
**Required shape — every summary does these three things, in this order:**

1. **Lead with the single most consequential development**, stated concretely in the first clause. No throat-clearing openers — "It was a busy day", "A heavy day in the courts", "Security realignment led the day", "Patch queues are long this cycle" all bury the news behind an abstraction. Open with the thing that happened.
2. **Group the rest into 2–3 themes**, not a list of events. The limit is **per sentence, not per summary: name at most 2 specific events in any one sentence.** A dense section may legitimately name six or seven things across five sentences — what it may never do is stack four of them into one sentence. Everything that isn't named gets characterised collectively ("half a dozen court cases", "the usual AI-tooling flaws").
3. **Close with the so-what or what's next — MANDATORY, not optional.** What this means for the reader, what it changes, or what to watch. A summary that ends on the last fact is unfinished.

Rules for the summary:
- **Length scales with the section's total volume.** Count **all** the items the section carries, not just story cards: `wars` counts its timeline entries across all theatres, `outbreaks` counts its outbreak entries plus its stories, the rest count story cards. Then:
  - **≤10 items** → 2–4 sentences, ~80 words
  - **11–20 items** → 3–4 sentences, ~110 words
  - **>20 items** → 4–5 sentences, up to ~150 words
  These are targets, not tripwires — overshooting by a few words to land a good closing thought is fine. What is NOT fine is spending the extra words on more names instead of more synthesis. A 150-word summary that is just a longer list is worse than an 80-word one that says something.
- **Tone:** Plain, conversational English. Like explaining the day's news to a friend over coffee. No jargon without unpacking it.
- **NO LIST-CHAINING (the most common failure).** Never chain three or more named events into one sentence with commas / "while" / "and". If you catch yourself writing *"Asia had a brutal day, with a Thai teenager killing seven, floods in Assam killing nearly 100 and Japan evacuating 260,000"*, that is a headline list wearing a sentence's clothes — collapse it to the theme plus ONE example: *"Three separate disasters hit Asia in a single day, the worst of them floods in Assam that killed nearly 100."* Synthesis means the sentence says something the individual cards don't.
  > **This rule is violated most often in the middle of a summary, not the opening** — the lead gets written carefully, then a "meanwhile, everything else" sentence turns into a dumping ground. Real example that slipped through: *"Washington drove most of the rest: new orders narrowing birthright citizenship, fresh solar and chip trade measures aimed at China, a US$1.2 billion payment to a German firm to abandon offshore wind, and a court ordering Meta to fund teen mental health."* Four named items in one sentence. Fix by naming the theme and keeping the strongest one or two: *"Washington drove most of the rest — trade measures aimed at China, a narrowed reading of birthright citizenship, and an unusual US$1.2 billion payment to kill an offshore wind project."* If more genuinely deserve naming, give them their own sentence.
- **Never restate the standing panels or stat tiles.** The Singapore weather strip and COE table, and the outbreak stat tiles, render **directly above** the summary — spending a sentence on "It is windy, 27–34 degrees" or re-quoting case counts wastes the reader's one paragraph. Interpret instead: trajectory, whether a number is rising or contained, whether it beats the last official count.
- **A story that leads two sections gets a different angle in each.** Big geopolitical events legitimately appear in both `top_stories` and `wars` (and a Singapore-relevant one in both `top_stories` and `singapore`). When that happens, the second summary must approach it from **that section's** vantage point, not repeat the first summary's sentence. Failure caught 2026-08-07: the Saudi–Türkiye–Pakistan defence pact opened both `top_stories` and `wars` with near-identical wording. `top_stories` should frame it as the day's dominant global development; `wars` should frame it as what it changes about the balance between the theatres it already tracks. Same fact, different question answered.
- **Section-specific framing:**
  - `top_stories`: the global mood / dominant story of the day
  - `singapore`: **lead with the hard news that affects daily life** (policy, advisories, cost, transport, courts). Now that trending items are uncapped, characterise the buzz **collectively in ONE clause** — "the talk is food-and-nostalgia flavoured this week, led by two long-running cafés closing" — rather than naming three unrelated viral items. Buzz never opens the summary and never takes more than a third of it.
  ...
  - `cybersecurity`: which threats matter most this cycle and why — patch-first framing (what a reader should act on)
...
- **Self-check before writing the JSON.** Re-read each summary and confirm: (a) it opens on a concrete development, not an abstraction; (b) **no single sentence names 3+ events** — check the middle sentences hardest, that is where the dumping ground forms; (c) its length matches the section's volume band, and any extra words are synthesis rather than more names; (d) it ends on a so-what; (e) it does not repeat the weather/COE/stat-tile numbers; (f) if a story also leads another section, the two summaries frame it differently. Rewrite any summary that fails.
...
> **Important:** `meta.generated_at` must be the **real current SGT clock time**, read fresh (`date -Iseconds`) immediately before writing the file. Never round it to the nearest 5 or 10 minutes, never write the scheduled slot time (09:00/15:00/21:00) instead of the actual finish time, and **never stamp a time in the future** — a forward-dated brief claims to be newer than it is, and the runner's freshness check will not catch it. (Failure caught 2026-08-07: a run that finished at 21:16 stamped `21:25`.)
...
> **CRITICAL — bundled-blob JSON safety (do not skip):** `world.html` is a bundled DC-app whose data lives in three JSON blobs embedded as the text of `<script type="__bundler/manifest|ext_resources|template">` tags. A JSON string inside one of those blobs must **never** contain a literal `</script>` — the browser's HTML parser would close the wrapping `<script>` early, truncate the blob, and the page dies with **"Error unpacking: Unterminated string in JSON"**. Always escape it as `</script>` (the slash as `/`), which `JSON.parse` decodes back to `</script>` while hiding it from the tokenizer. A `pre-commit` hook (`scripts/git-hooks/pre-commit` → `scripts/sanitize-world.mjs`) enforces this: it auto-escapes **every** literal `</` (not just `</script>`) in the blobs and **aborts the commit** if a blob still won't parse or the embedded `D` object no longer decodes. If your commit is rejected for this reason, regenerate `world.html` — do not bypass the hook. Note there are **two** ways this breaks: a literal `</script>` truncates the blob ("Error unpacking"); and — because the data sits in a `<script type="text/x-dc">` whose runtime is whitespace/markup-sensitive — leaving **other** closing tags literal (`</title>`, `</head>`, `</div>`, …) makes the x-dc runtime mis-parse the embedded `D` object, logging "sc-for … is not an array" and rendering an **unstyled page with visible `\n`**. The fix for both is the same: escape every `</` as `</`. The DC export does this natively; a hand-authored brief must too (the hook now enforces it).
